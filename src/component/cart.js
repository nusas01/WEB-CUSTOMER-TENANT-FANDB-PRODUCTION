...
function Cart({ closeCart }) {
    ...
    const { resetErrorPaymentMethodsCustomer } = getPaymentMethodsCustomerSlice.actions
    ...
    const handleCreateTransaction = () => {
        ...
        if (dataPaymentMethodCustomer.length === 0) {
            setMethodWithTransacion(true)
            dispatch(fetchPaymentMethodsCustomer())
            return
        }
    ...
    <div class="container-main-cart" style={{position: 'relative'}}>
        ...
        {toast && (
            <ToastPortal> 
                ...
                onClose={() => {
                    setToast(null)
                    dispatch(resetErrorPaymentMethodsCustomer())
                    dispatch(resetCreateTransactionCustomer())
                    setIsModelInputNumberEwallet(false)
                }} 
                duration={5000}
        ...
        <div>
            ...
            <div class="border-t">
                <div 
                    onClick={() => handleChoicePaymentMethodModel()} 
                    id="payment-method" 
                    className="flex p-6" 
                    style={{ margin: 'auto', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }}
                >
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        Metode Pembayaran {channelCode !== undefined ? `- ${channelCode}` : ""}
                        </span>
                    </div>
